...
echo "[TAG: RUNNING GLIBC-BENCH]"

echo "Starting glibc-bench..."
phoronix-test-suite batch-run glibc-bench >> $AUX_LOG_PATH 2>&1
...
echo "[TAG: AUX GLIBC-BENCH RESULTS]" >> "$AUX_LOG_PATH"
phoronix-test-suite result-file-to-text "$latest_result" >> "$AUX_LOG_PATH"
echo "[TAG: AUX GLIBC-BENCH RESULTS END]" >> "$AUX_LOG_PATH"

echo "[TAG: GLIBC-BENCH COMPLETE]"