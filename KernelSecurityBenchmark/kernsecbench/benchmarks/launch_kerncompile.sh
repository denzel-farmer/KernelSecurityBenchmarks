...
echo "[TAG: RUNNING KERNCOMPILE-BENCH]"

echo "Starting kerncompile-bench..."
echo 3 | phoronix-test-suite batch-run build-linux-kernel >> $AUX_LOG_PATH 2>&1
...
echo "[TAG: AUX KERNCOMPILE-BENCH RESULTS]" >> "$AUX_LOG_PATH"
phoronix-test-suite result-file-to-text "$latest_result" >> "$AUX_LOG_PATH"
echo "[TAG: AUX KERNCOMPILE-BENCH RESULTS END]" >> "$AUX_LOG_PATH"

echo "[TAG: KERNCOMPILE-BENCH COMPLETE]"
